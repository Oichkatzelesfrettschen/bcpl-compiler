#!/usr/bin/env python3
"""
OCODE to C Translator
Converts BCPL OCODE intermediate representation to portable C code.

OCODE format appears to be space-separated integers representing:
- Opcodes (operation codes)
- Labels (L followed by number)
- Operands (addresses, constants, etc.)

This provides a platform-agnostic solution for BCPL compilation,
eliminating the need for platform-specific assembly generation.
"""
...
class OCODETranslator:
    ...
    def translate_ocode(self, ocode_text: str) -> str:
        ...
        c_code += self.generate_c_runtime()
        ...
        # Generate goto labels for forward references
        for label in self.declared_labels:
            self.c_lines.append(f"    // Forward declaration: {label}")
        ...
        while i < len(tokens):
            ...
            if token.startswith('L'):
                ...
                opcode = int(token)
                next_i = self.process_opcode(tokens, i, opcode)
                # Check if we need to create a label for this position
                if next_i == i + 1 and opcode not in self.opcodes:
                    # This might be a label position, check if it's referenced
                    label_name = f"L{opcode}"
                    if label_name in self.declared_labels:
                        self.c_lines.append(f"    {label_name}:")
                i = next_i
                continue
...
'''
    ...
    def generate_c_runtime(self) -> str:
        """Generate BCPL runtime functions in C."""
        return '''
// BCPL I/O functions
...
int main(int argc, char* argv[]) {
    ...
    bcpl_store_global(0, argc);
    bcpl_store_global(1, (bcpl_word_t)(intptr_t)argv);
    
    // Call BCPL start function
    bcpl_start();
...
def main():
    if len(sys.argv) != 3:
        print("Usage: ocode_to_c.py input.O output.c", file=sys.stderr)
        sys.exit(1)
