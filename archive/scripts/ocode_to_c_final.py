#!/usr/bin/env python3
"""
OCODE to C Translator - Final Version
Converts BCPL OCODE intermediate representation to portable C code.

This version handles duplicate labels by making them unique and implements
a complete mapping of OCODE instructions to C operations.
"""
...
class OCODETranslator:
    ...
    def __init__(self):
        self.labels: Dict[str, str] = {}  # Original label -> unique label
        self.label_counter = 0
        self.c_lines: List[str] = []
        self.declared_labels: Set[str] = set()
        self.unique_labels: Set[str] = set()
        self.position_labels: Dict[int, str] = {}  # position -> unique label
        
        # Known opcodes with operand counts
        self.opcodes = {
            # Memory operations
            40: ("LOAD_LOCAL", 1),      # Load from local/stack
            41: ("LOAD_CONSTANT", 1),   # Load immediate constant  
            42: ("LOAD_INDIRECT", 1),   # Load through pointer
            81: ("STORE_LOCAL", 1),     # Store to local/stack
            91: ("LOAD_GLOBAL", 1),     # Load from global
            92: ("STORE_GLOBAL", 0),    # Store to global (no operand)
            
            # Control flow
            43: ("JUMP", 1),            # Unconditional jump
            44: ("JUMP_IF_ZERO", 1),    # Jump if top of stack is zero
            45: ("JUMP_IF_NOT_ZERO", 1), # Jump if top of stack is non-zero
            85: ("CALL", 1),            # Function call
            86: ("CONDITIONAL_JUMP", 1), # Conditional jump
            87: ("BRANCH_IF_NEQ", 1),   # Branch if not equal
            88: ("BRANCH_IF_EQ", 1),    # Branch if equal  
            89: ("BRANCH_IF_LT", 1),    # Branch if less than
            90: ("GOTO", 1),            # Jump/goto
            94: ("FUNCTION_START", 1),  # Function definition start
            95: ("FUNCTION_END", 1),    # Function definition end
            97: ("RETURN", 0),          # Return from function
            100: ("LOOP_START", 1),     # Loop start
            101: ("LOOP_END", 1),       # Loop end
            
            # Arithmetic operations
            8: ("ADD", 0),              # Addition
            9: ("SUBTRACT", 0),         # Subtraction
            10: ("MULTIPLY", 0),        # Multiplication
            11: ("DIVIDE", 0),          # Division
            12: ("MODULO", 0),          # Modulo operation
            14: ("COMPARE", 0),         # Comparison
            15: ("NEGATE", 0),          # Negation
            20: ("LESS_THAN", 0),       # Less than comparison
            
            # Stack operations
            51: ("DUPLICATE", 1),       # Duplicate top of stack
            68: ("POP", 0),             # Pop from stack
            79: ("PUSH_STRING", 0),     # Push string constant
            
            # I/O operations
            3: ("WRITE_CHAR", 0),       # Write character
            4: ("WRITE_STRING", 0),     # Write string
            5: ("READ_CHAR", 0),        # Read character
            6: ("READ_LINE", 0),        # Read line
            
            # Special operations
            49: ("FUNCTION_HEADER", 1), # Function header
            69: ("LOCAL_VAR", 0),       # Local variable allocation
            70: ("TEMP_VAR", 0),        # Temporary variable
            76: ("STRING_CONSTANT", 0), # String constant definition
            103: ("END_BLOCK", 0),      # End of block
        }
        
    def get_unique_label_for_position(self, original_label: str, position: int) -> str:
        """Get or create a unique label for the given original label at a specific position."""
        # Use position to make labels truly unique
        if position in self.position_labels:
            return self.position_labels[position]
        
        # Generate a unique label based on position
        unique_label = f"{original_label}_pos_{position}"
        self.position_labels[position] = unique_label
        return unique_label
    ...
    def translate_ocode(self, ocode_text: str) -> str:
        ...
        while i < len(tokens):
            ...
            if token.startswith('L'):
                # Label - make it unique by position
                unique_label = self.get_unique_label_for_position(token, i)
                ...
                # Unknown token
                self.c_lines.append(f"    // Unknown token: {token}")
                i += 1
    ...
    def process_opcode(self, tokens: List[str], index: int, opcode: int) -> int:
        ...
        if opcode in self.opcodes:
            op_name, operand_count = self.opcodes[opcode]
        else:
            op_name = f"UNKNOWN_{opcode}"
            operand_count = 0
        ...
        elif opcode in [43, 85, 86, 87, 88, 89, 90]:  # Jump/branch operations
            if index + 1 < len(tokens):
                target = tokens[index + 1]
                if target.startswith('L'):
                    # Find the position of this label in the token stream
                    target_pos = -1
                    for pos, tok in enumerate(tokens):
                        if tok == target:
                            target_pos = pos
                            break
                    unique_target = self.get_unique_label_for_position(target, target_pos)
                else:
                    # Numeric target, convert to label and find position
                    ...
                    for pos, tok in enumerate(tokens):
                        if tok == target_label:
                            target_pos = pos
                            break
                    unique_target = self.get_unique_label_for_position(target_label, target_pos)
                
                if opcode == 43:  # JUMP
                    self.c_lines.append(f"    goto {unique_target}; // JUMP")
                elif opcode == 85:  # CALL
                    self.c_lines.append(f"    goto {unique_target}; // CALL (as GOTO)")
                elif opcode == 86:  # CONDITIONAL_JUMP
                    self.c_lines.append(f"    if (bcpl_pop()) goto {unique_target}; // CONDITIONAL_JUMP")
                elif opcode == 87:  # BRANCH_IF_NEQ
                    self.c_lines.append(f"    {{ bcpl_word_t b = bcpl_pop(); bcpl_word_t a = bcpl_pop(); if (a != b) goto {unique_target}; }} // BRANCH_IF_NEQ")
                elif opcode == 88:  # BRANCH_IF_EQ
                    self.c_lines.append(f"    {{ bcpl_word_t b = bcpl_pop(); bcpl_word_t a = bcpl_pop(); if (a == b) goto {unique_target}; }} // BRANCH_IF_EQ")
                elif opcode == 89:  # BRANCH_IF_LT
                    self.c_lines.append(f"    {{ bcpl_word_t b = bcpl_pop(); bcpl_word_t a = bcpl_pop(); if (a < b) goto {unique_target}; }} // BRANCH_IF_LT")
                elif opcode == 90:  # GOTO
                    self.c_lines.append(f"    goto {unique_target}; // GOTO")
        ...
        # Special operations
            ...
            # Unknown or unimplemented opcode - consume it with proper operand count
            if operand_count > 0 and index + operand_count < len(tokens):
                operands = " ".join(tokens[index + 1:index + 1 + operand_count])
                self.c_lines.append(f"    // {op_name} {opcode} {operands} - not implemented")
                return index + 1 + operand_count
            else:
...
def main():
    if len(sys.argv) != 3:
        print("Usage: ocode_to_c_final.py input.O output.c", file=sys.stderr)
        sys.exit(1)
