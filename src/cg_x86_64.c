...
int rdn(void) {
  ...
  // Skip whitespace
  while (g_current_char == ' ' || g_current_char == '\t' ||
         g_current_char == '\n') {
 ...
 */
static void update_ch(void) { ch = g_current_char; }
...
static void ensure_buffer_capacity(size_t needed_size) {
  if (g_code_buffer.size + needed_size >= g_code_buffer.capacity) {
    ...
    g_code_buffer.buffer =
        realloc(g_code_buffer.buffer, g_code_buffer.capacity);
...
void emit(const char *format, ...) {
  ...
  // Write to buffer
  int written =
      vsnprintf(g_code_buffer.buffer + g_code_buffer.size,
                g_code_buffer.capacity - g_code_buffer.size, format, args);
...
void loadreg(int reg, int load_immediate) {
  ...
  default:
    ...
    emit("    // loadreg(%d, %d) - generic implementation", reg,
         load_immediate);
    break;
  }
 ...
 * @param label Label name
 */
void emit_label(const char *label) { emit("%s:", label); }
...
void emit_arithmetic(int op, int dest, int src1, int src2) {
  ...
  default:
    emit("    // arithmetic op %d: dest=%d, src1=%d, src2=%d", op, dest, src1,
         src2);
    break;
  }
}
...
void cg_set_debug_mode(bool enable) { g_debug_mode = enable; }
